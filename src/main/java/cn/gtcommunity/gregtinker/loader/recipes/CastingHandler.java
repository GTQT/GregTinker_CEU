...
import com.cleanroommc.groovyscript.compat.mods.tinkersconstruct.CastingTable;
...
public class CastingHandler {
    public static void init() {
        ...
        for (Material material : materials) {
            ...
            if (material.hasFlag(GENERATE_SMALL_GEAR)) CastingRecipe(material,OrePrefix.gearSmall, MetaItems.SHAPE_MOLD_GEAR_SMALL.getStackForm(),144);
            if (material.hasFlag(GENERATE_GEAR)) CastingRecipe(material,OrePrefix.gear, MetaItems.SHAPE_MOLD_GEAR.getStackForm(),576);
            if (material.hasFlag(GENERATE_PLATE))CastingRecipe(material,OrePrefix.plate, MetaItems.SHAPE_MOLD_PLATE.getStackForm(),144);
            if (material.hasFlag(GENERATE_BOLT_SCREW)) {
                CastingRecipe(material, OrePrefix.screw, GTQTMetaItems.SHAPE_MOLD_SCREW.getStackForm(), 18);
                CastingRecipe(material, OrePrefix.bolt, GTQTMetaItems.SHAPE_MOLD_BOLT.getStackForm(), 18);
            }
            if (material.hasFlag(GENERATE_ROD))CastingRecipe(material,OrePrefix.stick, GTQTMetaItems.SHAPE_MOLD_ROD.getStackForm(),72);
            if (material.hasFlag(GENERATE_RING))CastingRecipe(material,OrePrefix.ring, GTQTMetaItems.SHAPE_MOLD_RING.getStackForm(),36);
            if (material.hasFlag(GENERATE_LONG_ROD))CastingRecipe(material,OrePrefix.stickLong, GTQTMetaItems.SHAPE_MOLD_ROD_LONG.getStackForm(),144);
            if (material.hasFlag(GENERATE_ROTOR))CastingRecipe(material,OrePrefix.rotor, MetaItems.SHAPE_MOLD_ROTOR.getStackForm(),576);
            if (material.hasFlag(GENERATE_ROUND))CastingRecipe(material,OrePrefix.round, GTQTMetaItems.SHAPE_MOLD_ROUND.getStackForm(),18);
